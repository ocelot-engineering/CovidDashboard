...
get_daily_agg <- function(daily_cases, agg = sum, cols_sel = c("NEW_CASES", "NEW_DEATHS"), days_back = 365*2) {
    ...
    max_date <- daily_cases %>% dplyr::summarise(MAX_DATE = max(DATE_REPORTED)) %>% dplyr::pull(MAX_DATE)
    
    dat <- daily_cases %>% 
        dplyr::filter(DATE_REPORTED > max_date - days_back) %>% # only take past 2 years by default
